<?php
/**
 * The view file for edit function of quote module of RanZhi.
 */
?>
<?php include '../../common/view/header.html.php';?>
<?php include '../../../sys/common/view/chosen.html.php';?>
<div class='panel'>
  <div class='panel-heading'><strong><?php echo $lang->quote->edit . ' #' . $quote->code;?></strong></div>
    ...
    <form method='post' id='quoteForm' class='form-ajax'>
      <table class='table table-form w-p80'>
        <tr>
          ...
          <td><?php echo html::input('code', $quote->code, "class='form-control'");?></td>
          <th class='w-80px'><?php echo $lang->quote->customer;?></th>
          <td><?php echo html::select('customer', $customers, $quote->customer, "class='form-control chosen'");?></td>
        </tr>
        <tr>
          <th><?php echo $lang->quote->validUntil;?></th>
          <td><?php echo html::input('validUntil', $quote->validUntil, "class='form-control form-date'");?></td>
          <th><?php echo $lang->quote->discount;?></th>
          <td><?php echo html::input('discount', $quote->discount, "class='form-control text-right'");?></td>
        </tr>
        <tr>
          <th><?php echo $lang->quote->terms;?></th>
          <td colspan='3'><?php echo html::textarea('terms', $quote->terms, "class='form-control' rows='2'");?></td>
        </tr>
      </table>
      ...
      <table class='table table-bordered table-detail' id='detailTable'>
        ...
        <tbody>
          ...
          <tr>
            <td><input type='hidden' name='idList[<?php echo $i;?>]' value='<?php echo $item->id;?>'/><?php echo html::select("productList[{$i}]", $products, $item->product, "class='form-control chosen'");?></td>
            <td><?php echo html::input("quantityList[{$i}]", $item->quantity, "class='form-control text-right qty-input'");?></td>
            <td><?php echo html::input("priceList[{$i}]", $item->price, "class='form-control text-right price-input'");?></td>
            <td class='text-right amount-cell'><?php echo $item->amount;?></td>
